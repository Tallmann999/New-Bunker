...
public class EnemyShooting : MonoBehaviour
{
    ...
    {
        _enemyRotateDetector.EnemyShot += OnShooting;
    }
    ...
    private void OnShooting(bool shoot)
    {
    ...
    private void RotateToTarget(bool shootStatus)
    {
        if (shootStatus)
        {
            ...
            float angle = Mathf.Atan2(vectorToTarget.y, vectorToTarget.x)
                * Mathf.Rad2Deg - _rotationModific;

            Quaternion angleTarget = Quaternion.AngleAxis(angle, Vector3.forward);
            transform.rotation = Quaternion.Slerp(transform.rotation, angleTarget,
                Time.deltaTime * _shootingMovespeed);
        }
    }
}
