...
public class EnemyRotateDetector : MonoBehaviour
{
    ...
    public event UnityAction<bool> EnemyShot;
    ...
    {
        RaycastHit2D hit = Physics2D.Raycast(_rayDirection.position, 
            _rayDirection.TransformDirection(Vector3.down), _lazerLength);

        if (hit.collider != null && hit.collider.gameObject.TryGetComponent(out Player player))
        {
            EnemyShot?.Invoke(true);
            _isRotation = false;
        }
    }
